...
source_url: https://developers.cloudflare.com/registrar/troubleshooting/
source_file: src/content/docs/registrar/troubleshooting.mdx
license: CC-BY-4.0
...
- The domain has a status that blocks transfers (such as `serverHold` or `pendingDelete`). Refer to Domain is in a restricted status for details.
